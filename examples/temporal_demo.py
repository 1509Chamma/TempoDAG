...
def run_demo(output_dir: Path = OUTPUT_DIR) -> TemporalDemoReport:
    ...
    max_state_abs = max(
        ...
        for value in step.state.values()
    )
    max_output_abs = max(
        abs(value).max()
        for step in golden_trace.steps
    ...
    for reference_step, quantized_step in zip(
        ...
        max_state_abs_error = max(
            (
                float(
                    abs(
                        reference_step.state[name] - quantized_step.state[name]
                    ).max()
                )
...
def _state_to_quant_spec(spec: StateQuantSpec) -> QuantizationSpec:
    fixed_point = spec.fixed_point or FixedPointSpec(integer_bits=16, fractional_bits=8)
