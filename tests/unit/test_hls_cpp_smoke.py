...
def test_temporal_process_hls_emits_operator_kernels_at_file_scope() -> None:
    ...
    assert "void demo_process_step(const float stream_in" in rendered
    assert "#pragma HLS PIPELINE II=1" in rendered
    assert "rolling_mean_node_kernel<float, 4>(" in rendered
    assert "conv_node_kernel(" in rendered
    assert "head_add_kernel(" in rendered
