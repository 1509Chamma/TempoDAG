...
// Operator: FusedConv1DAddActivation
...
fused_conv_act_batch_loop:
  for (int batch = 0; batch < ${batch}; ++batch) {
fused_conv_act_out_channel_loop:
    for (int out_channel = 0; out_channel < ${out_channels}; ++out_channel) {
fused_conv_act_time_loop:
      for (int out_t = 0; out_t < ${output_length}; ++out_t) {
#pragma HLS PIPELINE II=1
        ${cpp_dtype} acc = bias[batch][out_channel][out_t];
fused_conv_act_in_channel_loop:
        for (int in_channel = 0; in_channel < ${in_channels}; ++in_channel) {
fused_conv_act_kernel_loop:
...
#pragma HLS UNROLL factor=2
        ...
        out[batch][out_channel][out_t] = ${activation_expression};
      }
    }
  }
}
