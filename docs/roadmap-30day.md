...
**Vision**: Build a credible temporal IR compiler foundation with streaming
state management and fixed-point verification that can compile a hybrid
time-series pipeline (rolling statistics + TCN/GRU) into parity-verified Vitis
HLS.

**Goal for Month 1**: Core temporal IR scaffolding + initial operator bridge +
verification layer entry point.
...
**Deliverable**: `src/tempo_dag/ir_temporal/` module with typed dataclasses and
validation

### 1.2 Update Documentation to Reflect Temporal Vision
- [x] Update `docs/architecture.md` to describe temporal layer above current
  kernel layer
- [x] Add `docs/temporal-ir-guide.md`: temporal concepts and invariants
- [x] Update `docs/development.md` with temporal operator patterns
- [x] Review and update `docs/roadmap.md` to align with stage-based model
  progression

**Deliverable**: Developer-ready documentation for temporal IR mental model

### 1.3 Rename & Reorganize Package
- [x] Verify all imports use `tempo_dag`
- [x] Update documentation to promote TempoDAG brand
- [x] Add `platform-vision.md` to docs
...
- [x] Update architecture documentation to show temporal layer
- [ ] Add sequence diagrams for temporal scheduling
- [x] Keep README in sync with implementation progress
- [x] Inline docstrings for all temporal IR classes
...
1. **Python-first for MVP**: Keep temporal IR in Python; MLIR migration is
   future work.
2. **ONNX focus**: Prioritize ONNX import; PyTorch FX/export as secondary path.
3. **Single-clock MVP**: Defer multi-clock support; assume single logical clock
   for first models.
4. **Exact optimization later**: Use greedy/heuristic scheduling for MVP; exact
   CP-SAT solver as next iteration.
5. **Fixed-point before floating**: Verification ladder starts with fixed-point
   oracle; float parity is post-MVP.
...
- **Finding 1** (Temporal DAG): Implemented in Week 1 with Edge0/EdgeDelta
  invariant.
- **Finding 2** (Stateful first-class): State/Buffer/Cache in Week 1-2;
  RollingBuffer/RollingMean in Week 2.
- **Finding 3** (Streaming optimization): Scheduling heuristics in Stage 2;
  initiation interval as primary metric.
- **Finding 4** (Fixed-point correctness): Fixed-point oracle and golden traces
  in Week 3; bit-exact gate in MVP.
- **Finding 5** (Prioritize streaming/DSP/GRU): MVP is rolling stats + conv
  (DSP-like); GRU in Stage 2.

---

## Blockers & Risks

- **Blocker**: If ONNX Scan semantics are unclear, add raw PyTorch imperative
  DSL path.
- **Risk**: Fixed-point oracle divergence from hardware; use HLS C simulation
  as bridge.
- **Risk**: State placement heuristic is suboptimal; add constraint-based solver
  in Stage 2.
- **Risk**: GRU lowering more complex than expected; scope to linear/additive
  first, defer non-linearity.
